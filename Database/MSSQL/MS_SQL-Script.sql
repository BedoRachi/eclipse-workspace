-- MSSQL은 Auto Commit이 됨 == Insert, update, delete시 자동으로 Commit이 됨

USE tempdb; -- 새로운 DB를 생성 또는 삭제시 워치 지정
GO -- SQL 명령어 실행(SSMS에서는 생략 가능하나 SQLCMD를 통해 할 경우 반드시 입력
...
(	userID char(8) NOT NULL PRIMARY KEY, -- 사용자 아이디
	name nvarchar(10) NOT NULL, -- 이름(유니코드 10자)
	birthYear int NOT NULL, -- 출생년도
	addr nchar(2) NOT NULL, -- 지역(경기,서울,경남 식으로 2글자만입력)(유니코드 2자)
	mobile1 char(3), -- 휴대폰의 국번(011, 016, 017, 018, 019, 010 등)(3 Byte)
...
(	num int IDENTITY NOT NULL PRIMARY KEY, -- 순번(PK)(Primary Key + 시퀀스(Identity))
	userID char(8) NOT NULL --아이디(FK)
	FOREIGN KEY REFERENCES userTbl(userID),--(Foreign Key)
